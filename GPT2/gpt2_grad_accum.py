...
B = 16
T = 1024
assert total_batch_size % (B * T) == 0, "make sure the batch size is divisible by B * T"
grad_accumulation_steps = total_batch_size // (B * T)
print(f"total desired batch size: {total_batch_size}")
print(f"calculated grad accumulation steps: {grad_accumulation_steps}")
...
for step in range(max_steps):
    ...
    t1 = time.time()
    dt = t1 - t0   # in secs
    tokens_processed = train_loader.B * train_loader.T * grad_accumulation_steps  # B * T * grad_accumulation_steps
